	...
	<view class="base">
		<scroll-view scroll-y style="height: 1600rpx;width: 100%;" @scrolltolower="reachBottom">
			<view class="moreannouncement">
				...
				<view class="Announcements">More Today</view>
				<!-- 三角形 -->
			    ...
			    <view class="nav-list">
			    	<view class="nav-li"
			    		 @click="MoreAnnouncementClick(index)"
			    		  v-for="(item,index) in TodayShowList" :key="index">
			    		<view class="rectangle_8">
			    			<view :class="[{'rectangle_91':index%2==0,
			    							'rectangle_92':index%2==1}]"></view>
			    		</view>
			    		 <view class="nav-title">{{TodayShowList[index]["Title"]}}</view>
			    	</view>
			    </view>
			</view>
		</scroll-view>
	...
	export default {
		...
		methods: {
			MoreAnnouncementClick(item1){
				var items;
				console.log(item1)
				items=encodeURIComponent(JSON.stringify(this.TodayShowList[item1]));
				uni.navigateTo({
